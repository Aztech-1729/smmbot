...
async def notify_order_created(bot: Bot, user_id: int, order: dict) -> None:
...
async def notify_deposit_approved(bot: Bot, user_id: int, deposit: dict) -> None:
    ...
    await notify_user(bot, user_id, text, check_enabled=False)
...
async def notify_new_deposit_to_admins(bot: Bot, deposit: dict) -> None:
    ...
    await notify_admins(bot, text, reply_markup=kb)


async def notify_new_ticket_to_admins(bot: Bot, ticket: dict) -> None:
...
async def notify_ticket_reply(bot: Bot, user_id: int, ticket: dict) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📋 View Ticket", callback_data=f"view_ticket:{str(ticket['_id'])}", style="primary")],
    ])
    await notify_user(bot, user_id, text, reply_markup=kb, check_enabled=False)
